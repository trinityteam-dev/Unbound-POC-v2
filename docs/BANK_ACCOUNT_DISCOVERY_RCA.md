...
- **2026-07-05 (verification + one more fix — Ghanshyam and Boulter Allen Super Funds):**
  Manually verified both funds' auto-discovered `bank_accounts` entries in `funds_config.json`
  directly against source statement OCR, given the two prior fabrication incidents above.
  **Ghanshyam:** BSB `182-512`, account `960362069`, "Wrap Cash Account" — confirmed exactly
  correct against `Ghanshyam_Macq CMA_Reimbursements-12.pdf` (a Macquarie wrap cash
  transaction list explicitly stating account name, BSB, account number, and product type).
  No change needed. **Boulter Allen:** BSB `182-222` was correct, but the configured account
  number `HRAADA` was wrong — traced its origin to
  `GCBoulterNomineesPL - Confirmation of Bank Account Details Amendment.pdf`, a MUFG/CHESS
  registry letter whose "Account Number" field is masked for that recipient and OCR'd the
  mask into letter-like garbage (`HRAAD A`). The account's own Macquarie CMA statements
  (`BASF - Macquarie Cash Management Account 30.8.2024.pdf`,
  `BASF - Macquarie Statement no. 196.pdf`, `GCBNPL - 2025 Macquarie Cash Management
  Account.pdf`) all clearly and consistently print `bsb 182 222` / `account no. 123484214` —
  a first-party bank statement is a more reliable source than a third-party registry notice
  with a masked field, and the number is corroborated identically across three independent
  statements spanning Aug 2024 to Jun 2025. Corrected `funds_config.json` to `123484214`
  (also renamed the account label from "Macquarie Bank Account" to "Macquarie Cash
  Management Account" to match the product name printed on the statement itself, same
  convention as Ghanshyam's "Wrap Cash Account"). **General takeaway reinforcing Root Cause
  #3:** a masked/partially-redacted account number in a registry or third-party
  confirmation letter is a plausible OCR-garbling trap distinct from the Customer-ID
  splicing seen in the A H Smith incidents — when a discovered account number doesn't look
  like a clean number (letters mixed with digits, inconsistent length, etc.), prefer
  whatever the account's own bank/platform statement states over a third-party document
